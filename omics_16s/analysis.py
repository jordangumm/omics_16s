...
class Analyzer(WorkflowRunner):
    ...
    def workflow(self):
        ...
        self.addTask("otu_generator", command=['python', self.otu_generator_fp, '--num_cpu', self.num_cpu,
                                                                  self.read_dp, self.dependencies_dp])
        self.addTask("taxass_analysis", command=['python', self.taxass_fp, '--num_cpu', self.num_cpu,
